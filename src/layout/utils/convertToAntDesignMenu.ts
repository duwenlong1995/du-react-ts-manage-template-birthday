 ...
 */
export const convertToAntDesignMenu = (routerData: any) => {
    // 内部递归函数，用于将单个路由项转换为 Ant Design 菜单项格式
    function convertRoute(route: any) {
        ...
        if (route.children && route.children.length > 0) {
            ...
            const validChildren = route.children
                .map((childRoute: any) => convertRoute(childRoute)) // 递归调用 convertRoute 处理每个子项
                .filter((child: any) => child !== null); // 过滤掉为 null 的子项
    ...
    const sortedMenu = routerData
        .map((route: any) => convertRoute(route))
