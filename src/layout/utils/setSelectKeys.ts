 ...
 */
function findVisibleItemByPath(routerData: any, targetPath: any) {
    // 递归查找函数
    function findRecursive(currentItems: any, currentPath: any) {
        for (const item of currentItems) {
            ...
            if (item.path === currentPath) {
                ...
                // 递归查找子项
                const childResult: any = findRecursive(item.children, currentPath);
                if (childResult) {
                    return childResult;
                }
            }
 ...
 */
const setSelectKeys = (routerData: any) => {
    // 获取当前路径
